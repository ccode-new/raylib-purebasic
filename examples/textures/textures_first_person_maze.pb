...
;- This example generates memory errors only on MacOS. Why?
...
Init_Vector3(@camera\target, 0.185, 0.4, 0.0)
...
CopyMemory(texture, model\materials\maps\texture + #MATERIAL_MAP_DIFFUSE, SizeOf(texture)) ; Set map diffuse texture

;Get Map image Data To be used For collision detection
Define *mapPixels = LoadImageColors(@imMap)
...
While Not WindowShouldClose() ; Detect window close button Or ESC key
  ...
  If IsKeyPressed(#KEY_F1)
    TakeScreenshot("screenshots/texture_first_person_maze.png")
  EndIf
