...
;-Update: 03.2025 (ccode_new - Free publication)
...
;CompilerIf #PB_Compiler_Backend = #PB_Backend_C
  ...
  DeclareModule ray
    ...
    Macro RL_CALLOC(n, sz)
      calloc(n,sz)
    EndMacro
    
    Macro RL_REALLOC(ptr, sz)
      realloc(ptr,sz)
    EndMacro
    ...
    ;// Material map type
    Enumeration MaterialMapType ;MaterialMapIndex
    ...
    Structure ModelAnimation Align #PB_Structure_AlignC
      ...
      *bones.BoneInfo              ;// Bones information (skeleton)
      Array *framePoses.rl_Transform(0)  ;// Poses array by frame
      name.a[32]
    EndStructure
    ...
    Structure FilePathList Align #PB_Structure_AlignC
      ...
      Array *paths.Character(0)               ;// Filepaths entries
    EndStructure
    
    ;// Automation event
    Structure AutomationEvent
      frame.rl_uint             ;// Event frame
      type.rl_uint              ;// Event type (AutomationEventType)
      params.rl_int[4]          ;// Event parameters (If required)
    EndStructure

    ;Automation event List
    Structure AutomationEventList
      capacity.rl_uint          ;// Events max entries (MAX_AUTOMATION_EVENTS)
      count.rl_uint             ;// Events entries count
    ...
    ImportC #pbhelper_path
      ImportC #raylib_path
        ...
        ToggleBorderlessWindowed()                                            RLAS(ToggleBorderlessWindowed)    ;// Toggle window state: borderless windowed, resizes window To match monitor resolution
        ...
        SetWindowIcon(*in_image.ray::Image)                                 __PBAS(SetWindowIcon)               ;// Set icon for window (single image, RGBA 32bit)
        SetWindowIcons(*in_images.ray::Image, count.rl_int)                 __PBAS(SetWindowIcons)              ;// Set icon for window (multiple images, RGBA 32bit)
        ...
        SetWindowMaxSize(width.rl_int, height.rl_int)                               RLAS(SetWindowMaxSize)            ;// Set window maximum dimensions (for FLAG_WINDOW_RESIZABLE)
        ...
        SetWindowFocused()                                                    RLAS(SetWindowFocused)            ;// Set window focused
        ...
        BeginMode3D(*in_camera3d.ray::Camera3D)                               RLAS(BeginMode3D)                 ;// Initializes 3D mode with custom camera (3D)
        ...
        LoadShader(*in_shader.ray::Shader, vsFileName.p-utf8, 
        ...
        LoadShaderFromMemory(*out_result.ray::Shader, vsCode.p-utf8, 
                             fsCode.p-ascii)                                __PBAS(LoadShaderFromMemory)        ;// Load shader from code strings And bind Default locations
        IsShaderValid.rl_bool(*in_shader.ray::Shader)                         RLAS(IsShaderValid)               ;// Check If a shader is valid (loaded on GPU)
        ...
        SetShaderValue(*in_shader.ray::Shader, locIndex.rl_int, *value, 
                       uniformType.rl_int)                                    RLAS(SetShaderValue)              ;// Set shader uniform value
        ...
        GetMouseRayEx(*out_result.ray::Ray, *in_mousePosition.ray::Vector2,
                    *in_camera.ray::Camera, width.rl_int, height.rl_int)    __PBAS(GetMouseRayEx)               ;// Get a ray trace from screen position (i.e mouse) in a viewport
        ...
        LoadAutomationEventList(*out_result.AutomationEventList, fileName.p-utf8)         __PBAS(LoadAutomationEventList)       ;// Load automation events List from file, NULL For empty List, capacity = MAX_AUTOMATION_EVENTS
        UnloadAutomationEventList(*in_list.AutomationEventList)                           __PBAS(UnloadAutomationEventList)     ;// Unload automation events List from file
        ExportAutomationEventList.rl_bool(*in_list.AutomationEventList, fileName.p-utf8)  __PBAS(ExportAutomationEventList)     ;// Export automation events list as text file
        SetAutomationEventList(*in_list.AutomationEventList)                              __PBAS(SetAutomationEventList)        ;// Set automation event List To record To
        SetAutomationEventBaseFrame(frame.rl_int)                                           RLAS(SetAutomationEventBaseFrame)   ;// Set automation event internal base frame To start recording
        StartAutomationEventRecording()                                                     RLAS(StartAutomationEventRecording) ;// Start recording automation events (AutomationEventList must be set)
        StopAutomationEventRecording()                                                      RLAS(StopAutomationEventRecording)  ;// Stop recording automation events
        PlayAutomationEvent(*event.AutomationEvent)                                         RLAS(PlayAutomationEvent)           ;// Play a recorded automation event
        ...
        IsKeyPressedRepeat(key.rl_int)                                        RLAS(IsKeyPressedRepeat)          ;// Check If a key has been pressed again
        ...
        SetGamepadVibration(gamepad.rl_int, leftMotor.rl_float, 
                            rightMotor.rl_float, duration.rl_float)           RLAS(SetGamepadVibration)         ;// Set gamepad vibration for both motors (duration in seconds)
...
;         UpdateCamera(*inout_camera.ray::Camera)                               RLAS(UpdateCamera)                ;// Update camera position for selected mode
        UpdateCamera(*in_camera.ray::Camera, cameraMode.rl_int)                 RLAS(UpdateCamera)                ;// Update camera position For selected mode
        
        UpdateCameraPro(*in_camera.ray::Camera, *movement.ray::_Vector3, 
                        *rotation.ray::_Vector3, zoom.rl_float)                 RLAS(UpdateCameraPro)             ;// Update camera movement/rotation
...
;         
        ...
        GetShapesTexture(*out_texture.ray::Texture2D)                       __PBAS(GetShapesTexture)            ;// Get texture that is used For shapes drawing
        GetShapesTextureRectangle(*out_rectangle.Rectangle)                 __PBAS(GetShapesTextureRectangle)   ;// Get texture source rectangle that is used For shapes drawing
...
;                             thick.rl_float, color.rl_ColorLong)             __PBAS(DrawLineBezierCubic)         ;// Draw line using cubic bezier curves with 2 control points
        ...
        DrawRectangleLinesEx(*in_rect.ray::Rectangle, lineThick.rl_float,
        ...
        DrawRectangleRoundedLines(*in_rect.ray::Rectangle,
                                  ...
                                  color.rl_ColorLong)                       __PBAS(DrawRectangleRoundedLines)   ;// Draw rectangle with rounded edges outline
        DrawRectangleRoundedLinesEx(*in_rect.ray::Rectangle, 
                                    roundness.rl_float, segments.rl_int, 
                                    lineThick.rl_float, color.rl_ColorLong) __PBAS(DrawRectangleRoundedLinesEx) ;// Draw rectangle with rounded edges outline
        ...
        DrawSplineLinear(*points.ray::Vector2, pointCount.rl_int, 
                         thick.rl_float, color.rl_ColorLong)                __PBAS(DrawSplineLinear)            ;// Draw spline: Linear, minimum 2 points
        DrawSplineBasis(*points.ray::Vector2, pointCount.rl_int, 
                        thick.rl_float, color.rl_ColorLong)                 __PBAS(DrawSplineBasis)             ;// Draw spline: B-Spline, minimum 4 points
        DrawSplineCatmullRom(*points.ray::Vector2, pointCount.rl_int, 
                             thick.rl_float, color.rl_ColorLong)            __PBAS(DrawSplineCatmullRom)        ;// Draw spline: Catmull-Rom, minimum 4 points
        DrawSplineBezierQuadratic(*points.ray::Vector2, pointCount.rl_int, 
                                  thick.rl_float, color.rl_ColorLong)       __PBAS(DrawSplineBezierQuadratic)   ;// Draw spline: Quadratic Bezier, minimum 3 points (1 control point): [p1, c2, p3, c4...]
        DrawSplineBezierCubic(*points.ray::Vector2, pointCount.rl_int, 
                              thick.rl_float, color.rl_ColorLong)           __PBAS(DrawSplineBezierCubic)       ;// Draw spline: Cubic Bezier, minimum 4 points (2 control points): [p1, c2, c3, p4, c5, c6...]
        DrawSplineSegmentLinear(*p1.ray::Vector2, *p2.ray::Vector2, 
                                thick.rl_float, color.rl_ColorLong)           RLAS(DrawSplineSegmentLinear)     ;// Draw spline segment: Linear, 2 points
        DrawSplineSegmentBasis(*p1.ray::Vector2, *p2.ray::Vector2, 
                               *p3.ray::Vector2, *p4.ray::Vector2, 
                               thick.rl_float, color.rl_ColorLong)            RLAS(DrawSplineSegmentBasis)      ;// Draw spline segment: B-Spline, 4 points
        DrawSplineSegmentCatmullRom(*p1.ray::Vector2, *p2.ray::Vector2, 
                                    *p3.ray::Vector2, *p4.ray::Vector2, 
                                    thick.rl_float, color.rl_ColorLong)       RLAS(DrawSplineSegmentCatmullRom) ;// Draw spline segment: Catmull-Rom, 4 points
        DrawSplineSegmentBezierQuadratic(*p1.ray::Vector2, *p2.ray::Vector2, 
                                         *p3.ray::Vector2, thick.rl_float, 
                                         color.rl_ColorLong)                  RLAS(DrawSplineSegmentBezierQuadratic)  ;// Draw spline segment: Quadratic Bezier, 2 points, 1 control point
        DrawSplineSegmentBezierCubic(*p1.ray::Vector2, *p2.ray::Vector2, 
                                     *p3.ray::Vector2, *p4.ray::Vector2, 
                                     thick.rl_float, color.rl_ColorLong)      RLAS(DrawSplineSegmentBezierCubic)  ;// Draw spline segment: Cubic Bezier, 2 points, 2 control points

        ;// Spline segment point evaluation functions, For a given t [0.0f .. 1.0f]
        GetSplinePointLinear(*out.ray::Vector2, *startPos.ray::Vector2, 
                             *endPos.ray::Vector2, t.rl_float)              __PBAS(GetSplinePointLinear)          ;// Get (evaluate) spline point: Linear
        GetSplinePointBasis(*out.ray::Vector2, *p1.ray::Vector2, 
                            *p2.ray::Vector2, *p3.ray::Vector2, 
                            *p4.ray::Vector2, t.rl_float)                   __PBAS(GetSplinePointBasis)           ;// Get (evaluate) spline point: B-Spline
        GetSplinePointCatmullRom(*out.ray::Vector2, *p1.ray::Vector2, 
                                 *p2.ray::Vector2, *p3.ray::Vector2, 
                                 *p4.ray::Vector2, t.rl_float)              __PBAS(GetSplinePointCatmullRom)      ;// Get (evaluate) spline point: Catmull-Rom
        GetSplinePointBezierQuad(*out.ray::Vector2, *p1.ray::Vector2, 
                                 *p2.ray::Vector2, *p3.ray::Vector2, 
                                 t.rl_float)                                __PBAS(GetSplinePointBezierQuad)      ;// Get (evaluate) spline point: Quadratic Bezier
        GetSplinePointBezierCubic(*out.ray::Vector2, *p1.ray::Vector2, 
                                  *p2.ray::Vector2, *p3.ray::Vector2, 
                                  *p4.ray::Vector2, t.rl_float)             __PBAS(GetSplinePointBezierCubic)     ;// Get (evaluate) spline point: Cubic Bezier
        ...
        CheckCollisionPointPoly.rl_bool(*point.ray::Vector2, 
                                        *points.ray::Vector2, 
                                        pointCount.rl_int)                  __PBAS(CheckCollisionPointPoly)     ;// Check If point is within a polygon described by Array of vertices
        ...
        LoadImageAnimFromMemory(*out_result.ray::Image, fileType.p-utf8, 
                                *fileData, dataSize.rl_int, *frames.Long)   __PBAS(LoadImageAnimFromMemory)     ;// Load image sequence from memory buffer
        ...
        IsImageValid.rl_bool(*in_image.ray::Image)                          __PBAS(IsImageValid)                ;// Check If an image is valid (Data And parameters)
        ...
        ExportImageToMemory(*out_result.p-utf8, *in_image.ray::Image, 
                            fileType.p-utf8, *fileSize.Long)                __PBAS(ExportImageToMemory)         ;// Export image to memory buffer
...
;                           left.rl_ColorLong, right.rl_ColorLong)            __PBAS(GenImageGradientH)           ;// Generate image: horizontal gradient
        GenImageGradientLinear(*out_image.ray::Image, width.rl_int, 
                               height.rl_int, direction.rl_int, 
                               start.rl_ColorLong, fin.rl_ColorLong)        __PBAS(GenImageGradientLinear)      ;// Generate image: linear gradient, direction in degrees [0..360], 0=Vertical gradient
        ...
        GenImageGradientSquare(*out_image.ray::Image, width.rl_int, 
                               height.rl_int, density.rl_float, 
                               inner.rl_ColorLong, outer.rl_ColorLong)      __PBAS(GenImageGradientSquare)      ;// Generate image: square gradient
        ...
        GenImageText(*out_image.ray::Image, width.rl_int, height.rl_int, 
                     text.p-utf8)                                           __PBAS(GenImageText)                ;// Generate image: grayscale image from text Data
        ...
        ImageFromChannel(*out_image.ray::Image, *in_sourceImage.ray::Image, 
                         selectedChannel.rl_int)                            __PBAS(ImageFromChannel)            ;// Create an image from a selected channel of another image (GRAYSCALE)
        ...
        ImageBlurGaussian(*inout_image.ray::Image, blurSize.rl_int)           RLAS(ImageBlurGaussian)           ;// Apply Gaussian blur using a box blur approximation
        ImageKernelConvolution(*inout_image.ray::Image, *kernel.Float, 
                               kernelSize.rl_int)                             RLAS(ImageKernelConvolution)      ;// Apply custom square convolution kernel To image
        ...
        ImageDrawLineEx(*inout_image.ray::Image, *in_start.ray::Vector2, 
                        *in_end.ray::Vector2, thick.rl_int, 
                        color.rl_ColorLong)                                 __PBAS(ImageDrawLineEx)             ;// Draw a line defining thickness within an image
        ...
        ImageDrawCircleLines(*inout_image.ray::Image, centerX.rl_int, 
                             centerY.rl_int, radius.rl_int, 
                             color.rl_ColorLong)                              RLAS(ImageDrawCircleLines)        ;// Draw circle outline within an image
        ImageDrawCircleLinesV(*inout_image.ray::Image, 
                              *in_center.ray::Vector2, 
                              radius.rl_int, color.rl_ColorLong)            __PBAS(ImageDrawCircleLinesV)       ;// Draw circle outline within an image (Vector version)
        ...
        ImageDrawTriangle(*inout_image.ray::Image, *v1.ray::Vector2, 
                          *v2.ray::Vector2, *v3.ray::Vector2, 
                          color.rl_ColorLong)                               __PBAS(ImageDrawTriangle)           ;// Draw triangle within an image
        ImageDrawTriangleEx(*inout_image.ray::Image, *v1.ray::Vector2, 
                            *v2.ray::Vector2, *v3.ray::Vector2, 
                            c1.rl_ColorLong, c2.rl_ColorLong, 
                            c3.rl_ColorLong)                                __PBAS(ImageDrawTriangleEx)         ;// Draw triangle With interpolated colors within an image
        ImageDrawTriangleFan(*inout_image.ray::Image, *points.ray::Vector2, 
                             pointCount.rl_int, color.rl_ColorLong)         __PBAS(ImageDrawTriangleFan)        ;// Draw a triangle fan defined by points within an image (first vertex is the center)
        ImageDrawTriangleStrip(*inout_image.ray::Image, 
                               *points.ray::Vector2, pointCount.rl_int, 
                               color.rl_ColorLong)                          __PBAS(ImageDrawTriangleStrip)      ;// Draw a triangle strip defined by points within an image
        ...
        IsTextureValid.rl_bool(*in_texture.ray::Texture2D)                    RLAS(IsTextureValid)              ;// Check if a texture is valid (loaded in GPU)
        UnloadTexture(*inout_texture.ray::Texture2D)                        __PBAS(UnloadTexture)               ;// Unload texture from GPU memory (VRAM)
        IsRenderTextureValid.rl_bool(*in_texture.ray::Texture2D)              RLAS(IsRenderTextureValid)        ;// Check If a render texture is valid (loaded in GPU)
        ...
        ColorIsEqual.rl_bool(col1.rl_ColorLong, col2.rl_ColorLong)            RLAS(ColorIsEqual)                ;// Check if two colors are equal
        ...
        ColorTint.rl_ColorLong(color.rl_ColorLong, tint.rl_ColorLong)         RLAS(ColorTint)                   ;// Get color multiplied with another color
        ColorBrightness.rl_ColorLong(color.rl_ColorLong, factor.rl_float)     RLAS(ColorBrightness)             ;// Get color With brightness correction, brightness factor goes from -1.0f To 1.0f
        ColorContrast.rl_ColorLong(color.rl_ColorLong, contrast.rl_float)     RLAS(ColorContrast)               ;// Get color with contrast correction, contrast values between -1.0f and 1.0f
        ...
        ColorLerp.rl_ColorLong(color1.rl_ColorLong, color2.rl_ColorLong, 
                               factor.rl_float)                               RLAS(ColorLerp)                   ;// Get color lerp interpolation between two colors, factor [0.0f..1.0f]
        ...
        IsFontValid.rl_bool(*out_font.ray::Font)                              RLAS(IsFontValid)                 ;// Check If a font is valid (font Data loaded, WARNING: GPU texture Not checked)
        ...
        SetTextLineSpacing(spacing.rl_int)                                    RLAS(SetTextLineSpacing)          ;// Set vertical line spacing when drawing With line-breaks
        ...
        CodepointsToUTF8.i(*codepoints.Long, length.rl_int)                   RLAS(TextCodepointsToUTF8)        ;// Encode text as codepoints array into UTF-8 text string (WARNING: memory must be freed!)
        ...
        DrawCubeWires(*in_position.ray::_Vector3, width.rl_float,
                      height.rl_float, length.rl_float, color.rl_ColorLong)   RLAS(DrawCubeWires)               ;// Draw cube wires
...
;                            color.rl_ColorLong)                              __PBAS(DrawCubeTextureRec)          ;// Draw cube with a region of a texture
        ...
        DrawCapsule(*startPos.ray::_Vector3, *endPos.ray::_Vector3, 
                    radius.rl_float, slices.rl_int, 
                    rings.rl_int, color.rl_ColorLong)                       __PBAS(DrawCapsule)                 ;// Draw a capsule With the center of its sphere caps at startPos And endPos
        DrawCapsuleWires(*startPos.ray::_Vector3, *endPos.ray::_Vector3, 
                         radius.rl_float, slices.rl_int, 
                         rings.rl_int, color.rl_ColorLong)                  __PBAS(DrawCapsuleWires)            ;// Draw capsule wireframe With the center of its sphere caps at startPos And endPos
        DrawPlane(*in_centerPos.ray::_Vector3, *in_size.ray::Vector2,
                  color.rl_ColorLong)                                         RLAS(DrawPlane)                   ;// Draw a plane XZ
        ...
        IsModelValid.rl_bool(*in_model.ray::Model)                            RLAS(IsModelValid)                ;// Check If a model is valid (loaded in GPU, VAO/VBOs)
        ...
        DrawModelPoints(*in_model.ray::Model, *position.ray::_Vector3, 
                        scale.rl_float, tint.rl_ColorLong)                  __PBAS(DrawModelPoints)             ;// Draw a model as points
        DrawModelPointsEx(*in_model.ray::Model, *position.ray::_Vector3, 
                          *rotationAxis.ray::_Vector3, rotationAngle.rl_float, 
                          *scale.ray::_Vector3, tint.rl_ColorLong)          __PBAS(DrawModelPointsEx)           ;// Draw a model As points With extended parameters
        ...
        ExportMesh.rl_bool(*in_mesh.ray::Mesh, fileName.p-utf8)             __PBAS(ExportMesh)                  ;// Export mesh Data To file, returns true on success
        ExportMeshAsCode.rl_bool(*in_mesh.ray::Mesh, fileName.p-utf8)       __PBAS(ExportMeshAsCode)            ;// Export mesh As code file (.h) defining multiple arrays of vertex attributes
        ...
        IsMaterialValid.rl_bool(*in_material.ray::Material)                   RLAS(IsMaterialValid)             ;// Check if a material is valid (shader assigned, map textures loaded in GPU)                                              // Check If a material is valid (shader assigned, Map textures loaded in GPU)
        ...
        UpdateModelAnimationBones(*in_model.ray::Model, 
                                  *in_anim.ray::ModelAnimation, 
                                  frame.rl_int)                             __PBAS(UpdateModelAnimationBones)   ;// Update model animation mesh bone matrices (GPU skinning)
        ...
        GetMasterVolume.rl_float()                                            RLAS(GetMasterVolume)             ;// Get master volume (listener)
        ...
        IsWaveValid.rl_bool(*in_wave.ray::Wave)                               RLAS(IsWaveValid)                 ;// Checks If wave Data is valid (Data loaded And parameters)
        ...
        LoadSoundAlias(*out_result.ray::Sound, *source.ray::Sound)          __PBAS(LoadSoundAlias)              ;// Create a new sound that shares the same sample Data As the source sound, does Not own the sound Data
        IsSoundValid.rl_bool(*sound.ray::Sound)                               RLAS(IsSoundValid)                ;// Checks If a sound is valid (Data loaded And buffers initialized)
        ...
        IsMusicValid.rl_bool(*in_music.ray::Music)                            RLAS(IsMusicValid)                ;// Checks If a music stream is valid (context And buffers initialized)
        ...
        IsAudioStreamValid.rl_bool(*in_stream.ray::AudioStream)               RLAS(IsAudioStreamValid)          ;// Checks If an audio stream is valid (buffers initialized)
        ...
        ;Declare AudioCallback(*bufferData, frames.rl_uint)
        
        ;*processor.AudioCallback -> Pointer to Procedure
        AttachAudioMixedProcessor(*processor)                                 RLAS(AttachAudioMixedProcessor)   ;// Attach audio stream processor to the entire audio pipeline, receives the samples as 'float'
        DetachAudioMixedProcessor(*processor)                                 RLAS(DetachAudioMixedProcessor)   ;// Detach audio stream processor from the entire audio pipeline
    ...
    Module ray
      ...
      Procedure UpdateLightValues(*shader.ray::Shader, *light.ray::Light)
        ...
        SetShaderValue(*shader, *light\typeLoc, @*light\type, #SHADER_UNIFORM_INT)
        ...
        SetShaderValue(*shader, *light\targetLoc, @target(), #SHADER_UNIFORM_VEC3)
        ...
        SetShaderValue(*shader, *light\colorLoc, @fcolor(), #SHADER_UNIFORM_VEC4)
      EndProcedure
